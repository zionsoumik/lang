...
class NDChild(object):
    ...
    def ns1Etrigger(self, s):
        if s.inflection != "Q" and "S" not in s.sentenceStr and s.outOblique():
    ...
    def nsEtrigger(self, s):
        if s.inflection == "DEC" and "S" not in s.sentenceStr and self.grammar["NT"] < 0.5:
            self.adjustweight("NS", 1, self.r)
            self.adjustweight("OPT", 1, self.r)
